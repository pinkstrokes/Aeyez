#!/usr/bin/env python3
"""
MMMU PRO inference script for Qwen2.5-VL models.
Usage: python infer/infer_qwen.py [MODEL_NAME] [MODE] [SETTING]
...
class MMMUProInferenceQwen:
    """MMMU PRO inference engine for Qwen models."""
    ...
    def save_results(self, results: List[Dict[str, Any]]):
        ...
        filename = f"mmmu_pro_{self.model_name}_{self.mode}_{self.setting.replace(' ', '_').replace('(', '').replace(')', '')}_{timestamp}.jsonl"
...
async def main():
    """Main function."""
    parser = argparse.ArgumentParser(description='MMMU PRO inference for Qwen models')
    parser.add_argument('model_name', help='Model name (e.g., qwen2.5-vl-3b)')
    ...
    # Initialize inference engine
    try:
        inference = MMMUProInferenceQwen(args.model_name, args.mode, args.setting)
    except Exception as e:
